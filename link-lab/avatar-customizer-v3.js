...
function ensureEditor(){const base=document.getElementById('avatarFinalBase');if(!base||!base.parentElement)return null;let panel=document.getElementById('avatarCustomizerV3');if(panel)return panel;panel=document.createElement('section');panel.id='avatarCustomizerV3';panel.innerHTML='<div class="avatar-v3-skin"></div><div class="avatar-v3-tabs"></div><div class="avatar-v3-options"></div><div class="avatar-v3-colors"></div><p class="avatar-v3-status"></p>';base.insertAdjacentElement('afterend',panel);const tabs=panel.querySelector('.avatar-v3-tabs');[['hair','PELO'],['accessory','ACCESORIOS']].forEach(([k,l])=>{const b=document.createElement('button');b.type='button';b.dataset.kind=k;b.textContent=l;b.onclick=()=>{tab=k;refresh(true)};tabs.append(b)});return panel}
...
function renderOptions(host){host.textContent='';const p=profile();for(const v of values(tab)){const b=document.createElement('button');b.type='button';b.dataset.kind=tab;b.className='avatar-v3-option'+(String(current(tab,p))===String(v)?' selected':'');const thumb=C()?.thumbFor?.(tab,v);if(thumb){thumb.className='avatar-v3-thumb';b.append(thumb)}const s=document.createElement('small');s.textContent=tab==='accessory'?LABELS[v]:String(v);b.append(s);b.onclick=()=>apply(tab==='accessory'?{customAccessory:v}:{[tab]:v});host.append(b)}}
