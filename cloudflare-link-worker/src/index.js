...
async function joinHeartbeat(env, request, roomId, joinId) {
  ...
  if (!(await requireJoin(env, request, join))) return bad(env, request, "No autorizado", 401);
  const now = Date.now();
  await env.DB.prepare("UPDATE room_joins SET updated_at = ?, expires_at = ? WHERE id = ? AND room_id = ?").bind(now, now + JOIN_TTL_MS, joinId, roomId).run();
...
export default {
  async fetch(request, env) {
    ...
    try {
      if (request.method === "GET" && path === "/v1/health") return response(env, request, { ok: true, service: "ml3d-link-lab", apiVersion: 1 });
