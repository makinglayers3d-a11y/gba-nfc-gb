(() => {
  ...
  const W=64,H=96,HAIR_W=32,HAIR_H=32;
  const ATLAS='./assets/avatar-modular/avatar-modular-atlas-v1.png?v=3';
  const HAIR_ATLAS='./assets/avatar-modular/hair-atlas-v1.png?v=1';
  ...
  let atlas=null,hairAtlas=null,ready=false,hairReady=false;
  const cache=new Map();
  ...
  function tintedHairFrame(variant,primary,accent){
    if(!hairReady||!hairAtlas||!variant)return null;
    const index=Math.max(0,Math.min(8,Number(variant)-1)),key=['hair-runtime',variant,primary,accent].join('|');
    ...
    ctx.drawImage(hairAtlas,(index*4)*HAIR_W,0,HAIR_W,HAIR_H,0,0,HAIR_W,HAIR_H);
    const im=ctx.getImageData(0,0,HAIR_W,HAIR_H),d=im.data,p=rgb(primary),a=rgb(accent||lighten(primary));
    for(let i=0;i<d.length;i+=4){
      if(d[i+3]<2)continue;
      const lum=(d[i]+d[i+1]+d[i+2])/3,t=Math.max(0,Math.min(1,(lum-105)/135)),shade=.62+(lum/255)*.68;
  ...
  img.src=ATLAS;
  const hairImg=new Image();hairImg.decoding='async';
  hairImg.onload=()=>{hairAtlas=hairImg;hairReady=true;cache.clear();install();window.ML3DAvatarCustomizerV3?.refresh?.(true)};
  hairImg.src=HAIR_ATLAS;
