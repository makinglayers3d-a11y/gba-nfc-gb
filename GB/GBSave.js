(() => {
  ...
  function saveCurrent(){
    if(!currentEmulator||!currentKey)return;
    try{if(typeof currentEmulator.getExtRam!=="function")return;const extRam=currentEmulator.getExtRam();if(!extRam||extRam.byteLength===0)return;localStorage.setItem(currentKey,JSON.stringify(Array.from(extRam)));}catch(error){console.error("No se pudo guardar la partida GB/GBC:",error);}
  }
  function loadFor(romPath,gbEmulator){const encoded=localStorage.getItem(saveKey(romPath));if(!encoded||!gbEmulator)return;try{const bytes=new Uint8Array(JSON.parse(encoded));if(typeof gbEmulator.loadExtRam==="function"){gbEmulator.loadExtRam(bytes);console.log("Partida GB/GBC restaurada:",romPath);}}catch(error){console.error("No se pudo restaurar la partida GB/GBC:",error);}}
  function startSaveTimer(){clearInterval(saveTimer);saveTimer=window.setInterval(saveCurrent,5000)}
